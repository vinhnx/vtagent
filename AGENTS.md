...
- **Integration Points**: Gemini API, tree-sitter parsers, PTY command execution, MCP tools (including context7 for enhanced memory/journaling)
...
- **Enhanced Context Awareness**: Connect to external MCP servers for enhanced context and tool capabilities
- **Context7 Integration**: Use `@upstash/context7-mcp` for advanced memory and journaling capabilities
- **Time Provider**: Use `mcp-server-time` for timezone conversions and time queries
- **Sequential Thinking**: Use `@modelcontextprotocol/server-sequential-thinking` for structured reasoning
- **Tool Integration**: MCP tools are automatically discovered and integrated with the same approval policies as built-in tools
...
- **Enable MCP in Configuration**: Set `mcp.enabled = true` in `vtcode.toml`
- **Use Context7 for Memory**: Configure `@upstash/context7-mcp` for enhanced context awareness
- **Leverage Time Provider**: Use `mcp-server-time` for accurate timezone handling
- **Tool Policy Integration**: MCP tools follow the same approval policies as built-in tools
- **Event Logging**: MCP tool execution is logged as message blocks in the chat interface
- **System Prompt Integration**: The agent is explicitly instructed to leverage MCP tools for enhanced context awareness, memory, and journaling whenever beneficial
