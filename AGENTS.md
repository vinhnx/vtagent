...
Key modules include:
- `gemini.rs` for AI integration
- `ui/` for terminal interfaces
- `commands/` for CLI functionality
- `tools.rs` for tool implementations
- `utils.rs` for common utility functions (newly refactored)
- `ast_grep.rs` for AST-based code operations
...
The agent now includes powerful AST-based code analysis tools:
- `ast_grep_search`: Syntax-aware pattern matching for code search
- `ast_grep_transform`: Safe code transformations using structural patterns
- `ast_grep_lint`: Rule-based code quality analysis
...
- `render_pty_output_fn`: Terminal output rendering
- `ProjectOverview`: Project information struct with helper methods
- `build_project_overview`: Extract project info from Cargo.toml and README.md
- `extract_toml_str`: TOML parsing utilities
- `extract_readme_excerpt`: README content extraction
- `summarize_workspace_languages`: Language analysis
