...
VTAgent opens each chat with a workspace-specific orientation generated by the context engine. Configure it with the `[agent.onboarding]` section:
...
When enabled, VTAgent prints the onboarding message before the first prompt, appends the same context block to the system prompt for the model, and shows the `chat_placeholder` hint above the initial `>` prompt so you always have a recommended next action ready.
