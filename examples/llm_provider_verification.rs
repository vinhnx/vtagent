...
fn test_provider_factory() {
    ...
    assert!(providers.contains(&"gemini".to_string()), "Should have Gemini");
    assert!(providers.contains(&"openai".to_string()), "Should have OpenAI");
    assert!(providers.contains(&"anthropic".to_string()), "Should have Anthropic");
    
    println!("✓ Provider factory created with {} providers", providers.len());
}
