...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("gpt-4o → {:?}", factory.provider_from_model("gpt-4o"));
    println!("claude-3-5-sonnet → {:?}", factory.provider_from_model("claude-3-5-sonnet"));
    println!("gemini-2.5-flash → {:?}", factory.provider_from_model("gemini-2.5-flash"));
    ...
    if let Ok(client) = anthropic_client {
        println!("✅ Anthropic client: {} ({})", client.model(), client.provider_name());
    }
