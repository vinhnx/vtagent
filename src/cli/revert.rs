...
pub async fn handle_revert_command(config: &CoreAgentConfig, turn: usize, partial: Option<String>) -> Result<()> {
    ...
    let file = config.workspace.join("snapshots").join(format!("turn_{}.json", turn));
    if !file.exists() {
        println!("Snapshot not found: {}", file.display());
        return Ok(());
    }
    let data = fs::read_to_string(&file)?;
    println!("Found snapshot file: {} ({} bytes)", file.display(), data.len());
    ...
    if let Some(p) = partial { println!("Requested partial revert: {} (not applied)", p); }
    Ok(())
}
