...
pub async fn handle_snapshots_command(config: &CoreAgentConfig) -> Result<()> {
    ...
    let manager = SnapshotManager::new(SnapshotConfig { directory: snap_dir, ..Default::default() });
    let snaps = manager.list_snapshots().await?;
    if snaps.is_empty() {
        println!("(none)");
    } else {
        for s in snaps {
            println!("- turn {}  size={}B  created={}  file={}", s.turn_number, s.size_bytes, s.created_at, s.filename);
        }
    }
    Ok(())
}
...
pub async fn handle_cleanup_snapshots_command(config: &CoreAgentConfig, max: Option<usize>) -> Result<()> {
    ...
    let snap_dir = config.workspace.join("snapshots");
    let mut cfg = SnapshotConfig { directory: snap_dir, ..Default::default() };
    if let Some(m) = max { cfg.max_snapshots = m; println!("Keeping maximum {} snapshots...", m); }
