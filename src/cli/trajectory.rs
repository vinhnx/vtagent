...
pub async fn handle_trajectory_command(_cfg: &CoreAgentConfig, file: Option<PathBuf>, top: usize) -> Result<()> {
    ...
    for line in reader.lines() {
        ...
        if let Ok(rec) = serde_json::from_str::<Rec>(&line) {
            match rec {
                Rec::Route { selected_model, class, multi_agent, ts, .. } => {
                    *class_counts.entry(class).or_insert(0) += 1;
                    *model_counts.entry(selected_model).or_insert(0) += 1;
                    if multi_agent { multi_agent_count += 1; }
                    total_routes += 1;
                    recent_timestamps.push(ts);
                }
                Rec::Tool { name, ok, ts, .. } => {
                    if ok { *tool_ok.entry(name).or_insert(0) += 1; }
                    else { *tool_err.entry(name).or_insert(0) += 1; }
                    total_tools += 1;
                    recent_timestamps.push(ts);
                }
            }
    ...
    println!("{} routes, {} tools, {} multi-agent sessions",
             style(total_routes).cyan(),
             style(total_tools).cyan(),
             style(multi_agent_count).cyan());
    ...
    if !class_counts.is_empty() {
        ...
        for (i, (k, v)) in classes.into_iter().take(top).enumerate() {
            let percentage = if total_class_usage > 0 { (v as f64) / (total_class_usage as f64) * 100.0 } else { 0.0 };
    ...
    if !model_counts.is_empty() {
        ...
        for (i, (k, v)) in models.into_iter().take(top).enumerate() {
            let percentage = if total_model_usage > 0 { (v as f64) / (total_model_usage as f64) * 100.0 } else { 0.0 };
    ...
    if !tool_ok.is_empty() || !tool_err.is_empty() {
        ...
        for (i, (name, ok, err, rate)) in tools.into_iter().take(top).enumerate() {
            ...
            println!("{:>2}. {:<20} {} ok: {:<4} err: {:<4} success: {:>5.1}%",
                     i+1, name, status, ok, err, rate*100.0);
        }
    }

    // Multi-agent usage
    if total_routes > 0 {
        let multi_agent_percentage = (multi_agent_count as f64) / (total_routes as f64) * 100.0;
        println!("\n{}", style("Multi-Agent Usage").bold());
        println!("Sessions using multi-agent: {} ({:.1}%)",
                 style(multi_agent_count).cyan(),
