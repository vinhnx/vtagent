...
async fn test_lmstudio_qwen_connection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("📤 Sending completion request to Qwen model: {}", qwen_model);
    ...
    if let Err(e) = completion_response {
        println!("❌ Failed to send completion request to {}: {}", qwen_model, e);
        return Err(e.into());
    }
    ...
    println!("✅ Received non-empty response from {}: {}", qwen_model, response_content);
...
fn test_lmstudio_json_parsing() {
    ...
    println!("✅ Sample LMStudio JSON response parsed successfully: {}", response_content);
}