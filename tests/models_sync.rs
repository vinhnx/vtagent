...
use vtagent_core::config::constants::{models, model_helpers};
...
fn constants_cover_models_json() {
    ...
    let models_data: Value = serde_json::from_str(&json)
        .expect("Failed to parse docs/models.json as valid JSON");

    let providers = models_data.as_object()
    ...
    for (provider_name, provider_spec) in providers {
        let provider_models = provider_spec.get("models")
        ...
        let model_ids_from_json: HashSet<&str> = provider_models.keys()
            .map(|s| s.as_str())
            .collect();
        ...
        if let Some(constants_models) = constants_models {
            let model_ids_from_constants: HashSet<&str> = constants_models.iter()
                .copied()
                .collect();
            ...
            assert!(
                ...
                provider_name, missing_in_constants, provider_name
            );
            ...
            assert!(
                ...
                provider_name, extra_in_constants, provider_name
            );
...
fn model_helpers_validation_edge_cases() {
    ...
    assert!(model_helpers::is_valid("lmstudio", models::lmstudio::DEFAULT_MODEL));
...
fn backwards_compatibility_constants() {
    ...
    assert_eq!(models::CLAUDE_SONNET_4_20250514, models::anthropic::CLAUDE_SONNET_4_20250514);
    assert_eq!(models::LMSTUDIO_QWEN_30B_A3B_2507, models::lmstudio::LMSTUDIO_QWEN_30B_A3B_2507);
}
