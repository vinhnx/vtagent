...
use vtagent_core::llm::{
    LLMProvider, LLMRequest, Message, MessageRole, ToolDefinition, ToolCall,
...
fn test_gemini_tool_call_format() {
    ...
    if let Ok(gemini_request) = result {
        ...
        assert_eq!(gemini_request["systemInstruction"]["parts"][0]["text"], "You are a helpful assistant.");
        
        // Check user message
        assert_eq!(contents[0]["role"], "user");
        assert_eq!(contents[0]["parts"][0]["text"], "What's the weather in New York?");
        ...
        assert_eq!(func_parts[0]["functionResponse"]["response"]["content"], "Sunny, 72°F");
        
        // Check tools definition
        let tools = gemini_request["tools"].as_array().unwrap();
        assert_eq!(tools[0]["functionDeclarations"][0]["name"], "get_weather");
        assert_eq!(tools[0]["functionDeclarations"][0]["description"], "Get weather for a location");
    }
}
