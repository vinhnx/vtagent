...
pub fn print_tool_policy_help() {
    ...
    println!();
    println!("Policies are stored in ~/.vtagent/tool-policy.json");
    println!("Once you approve or deny a tool, your choice is remembered for future runs.");
    println!();
    println!("Examples:");
    println!("  vtagent tool-policy status           # Show current policies");
    println!("  vtagent tool-policy allow read_file  # Allow read_file tool");
    println!("  vtagent tool-policy deny rm          # Deny rm tool");
    println!("  vtagent tool-policy reset-all        # Reset all to prompt");
}
