use super::provider::{LLMProvider, LLMError};
...
impl LLMFactory {
    pub fn new() -> Self {
        ...
        factory.register_provider("openai", Box::new(|api_key| {
            Box::new(OpenAIProvider::new(api_key)) as Box<dyn LLMProvider>
        }));
        
        factory.register_provider("anthropic", Box::new(|api_key| {
    ...
    pub fn create_provider(&self, provider_name: &str, api_key: String) -> Result<Box<dyn LLMProvider>, LLMError> {
        let factory_fn = self.providers.get(provider_name)
...
pub fn create_provider_for_model(model: &str, api_key: String) -> Result<Box<dyn LLMProvider>, LLMError> {
    ...
    let provider_name = factory.provider_from_model(model)
        .ok_or_else(|| LLMError::InvalidRequest(format!("Cannot determine provider for model: {}", model)))?;
