...
impl OpenAiBackend {
    pub fn new(api_key: String, model: String) -> Self {
        ...
        Self {
            ...
            model,
            http,
            last_tool_call_id_by_name: HashMap::new(),
        }
    }
    // Extract parts from a non-streaming OpenAI choice message
    fn extract_parts_from_choice(
        choice: &serde_json::Value,
        id_map: &mut std::collections::HashMap<String, String>,
    ) -> Vec<crate::gemini::Part> {
        use crate::gemini::{FunctionCall, Part};
        ...
        if let Some(msg) = choice.get("message") {
            ...
            if let Some(tool_calls) = msg.get("tool_calls").and_then(|x| x.as_array()) {
                for tc in tool_calls {
                    ...
                    if let Some(idv) = &id {
                        id_map.insert(name.clone(), idv.clone());
                    }
...
impl OpenAiBackend {
    async fn generate_content(
                ...
                match part {
                    ...
                    crate::gemini::Part::FunctionCall { function_call } => {
                        ...
                        let id = function_call.id.clone().unwrap_or_else(|| {
                            gen_id_counter += 1;
                            format!("call_{}", gen_id_counter)
                        });
                        self.last_tool_call_id_by_name
                            .insert(function_call.name.clone(), id.clone());
                        messages.push(json!({
                            "role": "assistant",
                            "content": "",
                            "tool_calls": [
        ...
        let v: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| anyhow::anyhow!("OpenAI parse failed: {}", e))?;
        let choice = v
            .get("choices")
            ...
            .unwrap_or(json!({}));
        let parts: Vec<Part> =
            Self::extract_parts_from_choice(&choice, &mut self.last_tool_call_id_by_name);
        ...
        F: FnMut(&str) -> Result<()>,
    {
        ...
        if let Some(cfg) = &req.generation_config {
            if let Some(t) = cfg.get("temperature").and_then(|v| v.as_f64()) {
                body["temperature"] = serde_json::json!(t as f32);
            }
            if let Some(m) = cfg.get("maxOutputTokens").and_then(|v| v.as_u64()) {
                body["max_tokens"] = serde_json::json!(m as u32);
            }
        }
        if let Some(tools) = &req.tools {
            let mut arr = Vec::new();
            for t in tools {
                for f in &t.function_declarations {
                    arr.push(serde_json::json!({"type":"function","function":{"name":f.name,"description":f.description,"parameters":f.parameters}}));
                }
            }
            if !arr.is_empty() {
                body["tools"] = serde_json::json!(arr);
            }
        }
...
impl AnthropicBackend {
    async fn generate_content(
        ...
        for c in &req.contents {
            ...
            if !content_arr.is_empty() {
                messages.push(json!({"role": role, "content": content_arr}));
            }
        }
        let system_text = req.system_instruction.as_ref().map(|s| {
            s.parts
                .iter()
                .filter_map(|p| p.as_text())
                .collect::<Vec<_>>()
        ...
        let v: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| anyhow::anyhow!("Anthropic parse failed: {}", e))?;
        let pieces = v
            .get("content")
            .and_then(|c| c.as_array())
            .cloned()
        ...
        for seg in pieces {
            if let Some(t) = seg.get("type").and_then(|x| x.as_str()) {
                match t {
                    "text" => {
                        if let Some(txt) = seg.get("text").and_then(|x| x.as_str()) {
                            parts.push(Part::Text {
                                text: txt.to_string(),
                            });
                        }
                    ...
                    "tool_use" => {
                        ...
                        if let Some(idv) = idc {
                            self.last_tool_call_id_by_name.insert(name, idv);
                        }
                    }
                    _ => {}
                }
        ...
        F: FnMut(&str) -> Result<()>,
    {
        ...
        for c in &req.contents {
            ...
            if !content_arr.is_empty() {
                messages.push(serde_json::json!({"role": role, "content": content_arr}));
            }
        }
        let mut body =
            serde_json::json!({"model": self.model, "messages": messages, "stream": true});
        ...
        if let Some(tools) = &req.tools {
            ...
            for t in tools {
                for f in &t.function_declarations {
                    arr.push(serde_json::json!({"name":f.name,"description":f.description,"input_schema":f.parameters}));
                }
            }
            if !arr.is_empty() {
                body["tools"] = serde_json::json!(arr);
                ...
                if let Ok(v) = serde_json::from_str::<serde_json::Value>(payload) {
                    match v.get("type").and_then(|x| x.as_str()) {
                            ...
                            if let Some(delta) = v.get("delta") {
                                match delta.get("type").and_then(|x| x.as_str()) {
                                    ...
                                    Some("input_json_delta") => {
                                        // Surface tool input JSON deltas during streaming
                                        if let Some(partial) =
                                            delta.get("partial_json").and_then(|x| x.as_str())
                                        {
                                            if !partial.is_empty() {
                                                let _ = on_chunk(partial);
                                            }
                                        }
                                    }
                                    _ => {}
                        ...
                        Some("content_block_start") => {
                            // If a tool_use block starts, emit an annotation for UI
                                ...
                                == Some("tool_use")
                            {
                                ...
                                let id = v
                                    ...
                                    .unwrap_or("");
                                let _ =
                                    on_chunk(&format!("[tool_use:start name={} id={}]", name, id));
                            }
                        }
                        Some("content_block_stop") => {
                            // Close tool_use annotation
                            if v.get("content_block")
                                .and_then(|b| b.get("type"))
                                .and_then(|t| t.as_str())
                                == Some("tool_use")
                            {
                                let _ = on_chunk("[tool_use:stop]");
                            }
                        }
                        _ => {}
                    }
